...
int main(int argc, char *argv[]) {
    int sizes[] = { 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608 }; // Array sizes from 1K to 8M
    ...
    for (int s = 0; s < numSizes; s++) {
        ...
        float *A = (float *)aligned_alloc(16, currentSize * sizeof(float));
        float *B = (float *)aligned_alloc(16, currentSize * sizeof(float));
        float *C = (float *)aligned_alloc(16, currentSize * sizeof(float));
        ...
        fprintf(stderr, "%12d\t", currentSize);

        double maxPerformance = 0.0;
        for (int t = 0; t < NUMTRIES; t++) {
            double time0 = omp_get_wtime();
            NonSimdMul(A, B, C, currentSize);
            double time1 = omp_get_wtime();
            double perf = (double)currentSize / (time1 - time0);
            if (perf > maxPerformance)
                maxPerformance = perf;
        }
        double megaMults = maxPerformance / 1000000.0;
        ...
        for (int t = 0; t < NUMTRIES; t++) {
            double time0 = omp_get_wtime();
            SimdMul(A, B, C, currentSize);
            double time1 = omp_get_wtime();
            double perf = (double)currentSize / (time1 - time0);
            if (perf > maxPerformance)
                maxPerformance = perf;
        }
        megaMults = maxPerformance / 1000000.0;
        ...
        for (int t = 0; t < NUMTRIES; t++) {
            double time0 = omp_get_wtime();
            sumn = NonSimdMulSum(A, B, currentSize);
            double time1 = omp_get_wtime();
            double perf = (double)currentSize / (time1 - time0);
            if (perf > maxPerformance)
                maxPerformance = perf;
        }
        double megaMultAdds = maxPerformance / 1000000.0;
        ...
        for (int t = 0; t < NUMTRIES; t++) {
            double time0 = omp_get_wtime();
            sums = SimdMulSum(A, B, currentSize);
            double time1 = omp_get_wtime();
            double perf = (double)currentSize / (time1 - time0);
            if (perf > maxPerformance)
                maxPerformance = perf;
        }
        megaMultAdds = maxPerformance / 1000000.0;
