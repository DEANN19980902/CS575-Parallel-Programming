...
main( int argc, char *argv[ ] )
{
        ...
        for( int times = 0; times < NUMTIMES; times++ )
        {
            ...
            for( int n = 0; n < NUMTRIALS; n++ )
            {
                ...
                float vx  = sqrt(2.f * (beforey - aftery) * GRAVITY);  // time to fall from aftery to the ground
                float t = sqrt(2.f * aftery / GRAVITY);                                // approx horizontal velocity to reach distx
                float dx  = vx * t;                                  // should equal distx if our assumptions are correct

                if( fabs(dx - distx) <= RADIUS )                     // check if within the radius of the cup
                    numSuccesses++;
